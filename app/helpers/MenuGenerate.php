...
class MenuGenerate
{
    ...
    public function list_menu_by_modul( $root_id = 0 )
    {
        $html  = '';
        $items = $this->fetchMenu;
        ...
        while ( $loop && ( ( $option = each( $children[$parent] ) ) || ( $parent > $root_id ) ) )
        {
            ...
            elseif ( !empty( $children[$option['value']['menu_id']] ) )
            {
                ...
                $html .= sprintf(
                    ...
                    <a class="" href="/%4$s/%5$s"><span class="site-menu-title">%3$s</span></a>',
                    $tab,   // %1$s = tabulation
                    $option['value']['menu_id'],
                    $option['value']['menu_title'],
                    $option['value']['resources_name'],
                ...
                $html .= $tab . "\t" . '<ul class="site-menu-sub">';
                ...
                // HTML for menu item with no children (aka "leaf")  
                $html .= sprintf(
                    '%1$s<li class="site-menu-item  active">
                        <a href="/%4$s/%5$s" data-dropdown-toggle="false">
                        <i class="site-menu-icon md-account" aria-hidden="true"></i>
                        <span class="site-menu-title ">%3$s</span>
                        </a>
                    </li>',
                    str_repeat( "\t", ( count( $parent_stack ) + 1 ) * 2 - 1 ),   // %1$s = tabulation
                    $option['value']['menu_id'],
    ...
    public function nesTable( $root_id = 0 )
    {
        ...
        while ( $loop && ( ( $option = each( $children[$parent] ) ) || ( $parent > $root_id ) ) )
        {
            ...
            elseif ( !empty( $children[$option['value']['menu_id']] ) )
            {
                ...
                $html .= sprintf(
                    '
                    %1$s<li class="dd-item " data-id="%2$s">
                    <div class="dd-handle">
                        <span id="label_show%2$s">%3$s</span>
                        <span class="pull-right text-warning">/<span id="link_show%2$s">%4$s/%5$s</span> &nbsp;&nbsp; 
                ...
                $html .= sprintf(
                    ...
                    <div class="dd-handle">
                        ...
                        <span class="pull-right text-warning">/<span id="link_show%2$s">%4$s/%5$s</span> &nbsp;&nbsp; 
                            <!-- 
                            <a class="edit-button" id="%2$s" resources="%4$s" href="/alsus/index" access="%5$s" parent="%6$s">edit</i></a>
                            -->
                        </span>
                    </div>
                    </li>',
	...
	public function unitOrg( $root_id = 0 )
    {
        $html  = '';
        $items = $this->fetchMenu;
        ...
        while ( $loop && ( ( $option = each( $children[$parent] ) ) || ( $parent > $root_id ) ) )
        {
            ...
            elseif ( !empty( $children[$option['value']['id_unit_kerja']] ) )
            {
                $tab = str_repeat( "\t", ( count( $parent_stack ) + 1 ) * 2 - 1 );
                ...
                $html .= sprintf(
                    ...
                    $option['value']['nama_unit_kerja'],
                    $option['value']['alamat'],
                    $option['value']['keterangan'],
                    $option['value']['id_parent']
                ); 
                $html .= $tab . "\t" . '<ol class="child">';

                array_push( $parent_stack, $option['value']['id_parent'] );
                $parent = $option['value']['id_unit_kerja'];
            }
            else
                // HTML for menu item with no children (aka "leaf") 
                $html .= sprintf(
                    ...
                    %1$s<li class="dd-item" data-id="%2$s">
                    <div class="dd-handle">
						<i class="site-menu-icon icon fa-angle-right" aria-hidden="true"></i>
                        <span id="label_show%2$s">%3$s</span>
                        <span class="pull-right text-warning">/<span id="link_show%2$s">%4$s/%5$s</span> &nbsp;&nbsp; 
                            <a class="edit-button" id="%2$s" resources="%4$s" access="%5$s" parent="%6$s"><i class="glyphicon glyphicon-pencilx"></i></a>
                        </span>
