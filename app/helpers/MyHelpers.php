...
class MyHelpers
{
	...
	public function list_status($def=false)
    {
        $form=new Form();
        $result=[];
        $result['0']='Tidak Aktif';
        $result['1']='Aktif';
        //$result['name']=array('a','b');
        $select = new Select("list_status",$result,["useEmpty"  =>  true,"emptyText" =>  "-"]);
        if($def != false){
    ...
    public function ymd_to_str_indo($ymd)
    {
        $month=[
            1=>'Januari',
            2=>'Februari',
        ...
        // 1988-11-14
        return substr($ymd,8,2).' '.$month[(int) substr($ymd,5,2)].' '.substr($ymd,0,4);
    }
