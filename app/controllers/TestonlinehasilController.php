...
class TestonlinehasilController extends ControllerBase
{
    protected $apiOut;
	...
	{ 
		if (strlen($this->session->get('auth')->sekolah) > 0) {
            ...
            if ($this->session->get('auth')->roles=='siswa') {
                $builder = $this->modelsManager->createBuilder()  
                    ->columns('
                         TestOnlineSiswa.id
                        ,TestOnlineSiswa.sekolah
                        ,TestOnlineSiswa.id_mapel
                        ,RefMapel.mapel
                        ,TestOnlineSiswa.tingkat
                        ,TestOnlineSiswa.rombel
                        ,TestOnlineSiswa.tgl_test
                        ,TestOnlineSiswa.jam_mulai
                        ,TestOnlineSiswa.durasi
                        ,TestOnlineSiswa.jam_berakhir
                        ,TestOnlineSiswa.created_at
                        ,TestOnlineSiswa.created_by
                        ,TestOnlineSiswa.updated_at
                        ,TestOnlineSiswa.updated_by
                        ,TestOnlineSiswa.siswa_skor
                        ,Siswa.nama_lengkap
                        ,Siswa.nisn
                        ,Siswa.nik
        
                    ')
                    ->where("TestOnlineSiswa.sekolah = '".$this->session->get('auth')->sekolah."' and TestOnlineSiswa.siswa_nisn = '".$this->session->get('auth')->username."'")
                    ->Join('RefMapel','TestOnlineSiswa.id_mapel=RefMapel.id','RefMapel')
                    ->Join('Siswa','TestOnlineSiswa.siswa_nisn=Siswa.nisn','Siswa')
                    ->from('TestOnlineSiswa');
            }
            else {
                $builder = $this->modelsManager->createBuilder()  
                    ->columns('
                    ...
                    ')
                    ->where("TestOnlineSiswa.sekolah = '".$this->session->get('auth')->sekolah."'")
                    ->Join('RefMapel','TestOnlineSiswa.id_mapel=RefMapel.id','RefMapel')
                    ->Join('Siswa','TestOnlineSiswa.siswa_nisn=Siswa.nisn','Siswa')
                    ->from('TestOnlineSiswa');            
            }
	...
	{ 
		if($id!=false) {
			$row=TestOnlineSiswa::findFirstById($id);
            $old=$row;
			if($row) {
				$data['row']=$row;
				$data['soal_foto']="-";
