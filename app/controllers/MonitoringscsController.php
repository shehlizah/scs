...
class MonitoringscsController extends ControllerBase
{
    protected $apiOut;
    ...
    public function indexAction($param=false)
    {
        if ($this->request->isAjax()) {
            ...
            $query = $this->modelsManager->createBuilder()
                ...
                ->from('TaskPic')
                ->where('TaskPic.date_action LIKE :date_range:', ['date_range' => ''.$pick_ym.'-%'])
                ->orderBy('RefSekolah.kab_kota asc, RefSekolah.urutan asc')
                ->getQuery();

            $task_pic = $query->execute();
            $data['task_pic'] = $task_pic;
