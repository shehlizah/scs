...
class GuruController extends ControllerBase
{
    ...
    public function indexAction($param=false)
    {
        ...
        if($this->request->isPost()){
            if($this->request->getPost('simpancapture') == 'simpan') {
                ...
                $siswa=Guru::findFirstById($this->request->getPost('id'));
                    ...
                    'data'=>[
                        'FaceInfo'=>[
                            0=>[
                                ...
                                "Feature" => "",
                                "FtVersion" => 141188,
                                "Name" => $siswa->nama_lengkap.' '.$siswa->nuptk.' '.$siswa->sekolah,
                                "Country" => $siswa->provinsi,
                                "NativePlace" => $siswa->kabkota,
                                "IdCode" => $siswa->tgllahir,
                                "Job" => "GURU",
                                // "Phone" => $siswa->nik,
                ...
                if($_resp['data']['Count'] == 1 && $_resp['data']['ResultReason'][0] == 'Successful' && strlen($_resp['data']['Id'][0]) > 0 && strlen($_resp['data']['MD5'][0]) > 0) {
                    $upload_dir = 'memberdir/'.str_replace(" ","",$siswa->sekolah).'_GURU';
                    ...
                    $file = $upload_dir . '/' . $siswa->nuptk . '.jpg';
                    // $file = $upload_dir . '/0075917330.jpg';
                    $success = file_put_contents($file, $data);

                    $e=Guru::findFirstById($this->request->getPost('id'));
                    ...
                    $this->response->redirect("/guru/?skelas=".$this->request->get('skelas')."&snamasiswa=".$this->request->get('snamasiswa')."&page=".$this->request->get('page'));
                } else {
    ...
    public function createAction($param=false)
    {
        $out=[];
        $currentPage = $this->request->getQuery('page', 'int');
        $varPost=[];
        ...
        {            
            $row=new Guru();
			$row->provinsi='SULAWESI SELATAN';
			$row->kabkota='KABUPATEN GOWA';
			$row->sekolah='SMAN 1 GOWA';
			$row->tahun_ajaran='2023 Semester Genap';
			...
			$row->sex=$this->request->getpost('jenkel');
			...
			$row->agama=$this->request->getpost('agama');
			...
			$row->aktif=$this->request->getpost('aktif');
            $row->created_at=date('Y-m-d H:i:s');
			$row->created_by=$this->session->get('auth')->username;	
            $row->save();
            
            $this->flashSession->success("Simpan Data Guru Berhasil"); 
            $this->response->redirect("/guru");

            // $this->flashSession->error("Simpan Data Gagal"); 
