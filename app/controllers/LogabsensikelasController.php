...
class LogabsensikelasController extends ControllerBase
{
    protected $apiOut;
	...
	{ 
        ...
        if (strlen($this->session->get('auth')->sekolah) > 0) {
            $builder = $this->modelsManager->createBuilder()  
                ->columns('
                    AbsensiKelas.uuid,
                    AbsensiKelas.uuid_siswa,
                    AbsensiKelas.sekolah,
                    AbsensiKelas.nisn,
                    AbsensiKelas.tgl_absen,
                    AbsensiKelas.jam_absen,
                    AbsensiKelas.nama_siswa,
                    AbsensiKelas.kelas,
                    AbsensiKelas.status,
                    AbsensiKelas.waktu,
                    AbsensiKelas.area,
                    AbsensiKelas.terminal_sn,
                    AbsensiKelas.upload_time,
                    AbsensiKelas.photo_url,
                    AbsensiKelas.absen_id,
                    AbsensiKelas.created_at,
                    AbsensiKelas.sync_at,
                    AbsensiKelas.notif_sync_at,
                    AbsensiKelas.notif_sync_status,
                    Siswa.image_path
                ')
                ->Join('Siswa','AbsensiKelas.uuid_siswa=Siswa.id','Siswa')
                ->where("AbsensiKelas.tgl_absen = '".$pick_date."' and AbsensiKelas.sekolah = '".$this->session->get('auth')->sekolah."'")
                ->from('AbsensiKelas');            
        }
        else {
            $builder = $this->modelsManager->createBuilder()  
                ...
                ')
                ->Join('Siswa','AbsensiKelas.uuid_siswa=Siswa.id','Siswa')
                ->where("AbsensiKelas.tgl_absen = '".$pick_date."'")
                ->from('AbsensiKelas'); 
        }
